...
if [ ! -f results_$fields/interaction_${itest}.txt ]; then
    ...
    python gen_heatmap_ld.py results_$fields/interaction_${itest}.txt | tee results_$fields/interaction_${itest}_ld.txt
    cat results_$fields/interaction_${itest}.txt | tr -s " " | sed 's/^ //' | sed 's/ /\t/g' | awk '{print $1","$2","$7","$8","$9","$10}' > results_$fields/interaction_${itest}_plot_data.csv
    echo -e "set term png\nset out \"results_$fields/interaction_${itest}.png\"\nset xla 'Loci'\nset title 'Results of interaction test with $itest on $fields fields'\nset yla 'Loci'\nset xtics rotate by -90\nset grid xtics ytics\nset palette defined ( 0 \"red\", 1 \"white\", 2 \"green\" )\nset cbrange [-10:10]\nunset colorbox\nplot \"results_$fields/interaction_${itest}_graph.txt\" matrix rowheaders columnheaders u 1:2:3 w image title \"\",\"results_$fields/interaction_${itest}_ld.txt\" matrix rowheaders columnheaders u 1:2:( \$3 == 0 ? \"\" : \$3 == 1 ? \"+\" : \$3 == 2 ? \"-\" : \"+-\" ) w labels title \"\"\nset term svg\nset out \"results_$fields/interaction_${itest}.svg\"\nrep\nset term qt\nset out\nrep" > vis.plt
    gnuplot -p -c vis.plt
    rm vis.plt
fi
